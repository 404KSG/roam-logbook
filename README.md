...
Current package version: **0.9.0-beta.13**. This is a beta fork; the graph remains
...
**Dashboard** — `Logbook: Open dashboard`, or the button in the popover. The Roam Logbook Dashboard is a content-fit Linear-inspired inspector: one compact horizontal summary strip with Today, the selected range, and Tasks tracked, followed by Running only when present and a primary By Task list panel. The strip keeps one subtle outer frame, transparent metric cells, and a 26–30px accessible activity rail made from the real daily buckets; exact date/duration remains in each bucket's accessible name and tooltip. Desktop uses three columns, 600–719px keeps a compact three-column strip, and narrower screens place Today/Tasks on the first row with Last 7 days below without horizontal page scroll. The overlay is fixed to the viewport, locks background document scroll while open, and keeps only the dialog body scrollable; closing, Escape, overlay click, and extension unload restore the original document styles and scroll position. Short reports end shortly after their last row; long reports scroll inside the body while the header and controls remain available. Finite ranges expose their real buckets; All time keeps the exact total but labels the chart as a recent 30-day activity window. The roll-up explanation is available through the By Task info control instead of a full-width footer note. The surface uses quiet borders, spacing, and hierarchy instead of heavy cards, independent By Day sections, or border soup.
